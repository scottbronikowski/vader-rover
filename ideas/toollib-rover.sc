...
(define (get-next-image-command)
 (let* ((image1 (rover-get-front-cam)) 
       ...
       (scaled-image1 (imlib:create-scaled image1 320 240))
       (scaled-image2 (imlib:create-scaled image2 320 240))
       (tempvar (read-cdr-viewer-active)))
  (draw-imlib-pixmap scaled-image1 0 0)
  (draw-imlib-pixmap scaled-image2 330 0)
  (imlib:free image1)
  (imlib:free image2)
  (imlib:free scaled-image1)
  (imlib:free scaled-image2)
  (if (= tempvar 1)
...
(define (cdr-get-next-image-command)
 (let ((image1 (cdr-get-front-cam)) 
  ...
  (draw-imlib-pixmap image1 0 0)
  (draw-imlib-pixmap image2 660 0)
  (imlib:free image1)
  (imlib:free image2)
  ;; (if (= tempvar 1)
