...
(define (model-threshold model-name model-path) (panic "matlab"))
...
(define (get-bss boxes-movie)
 (map (lambda (boxes-per-frame)
       (map (lambda (box)
	     (let* ((box-c (voc4-detection->box box))
		    (score (log ((c-function
				  double
				  ("box_single_score" pointer))
				 box-c))))
	      (free-box box-c)
	      score))
	    boxes-per-frame))
...
(define (get-bps boxes-movie flow-movie scale)
 (map (lambda (boxes-t boxes-t+1 flow-t)
       (let* ((c-flow-struct-t (optical-flow->c-flow-struct flow-t))
	      (scores (map-indexed (lambda (box-t i)
			    (map-indexed (lambda (box-t+1 j)
				  (let* ((box-t-c (voc4-detection->box box-t))
					 (box-t+1-c (voc4-detection->box box-t+1))
					 (score (log ((c-function
						       double
						       ("box_pair_score" pointer pointer pointer double))
						      box-t-c box-t+1-c c-flow-struct-t scale))))
				   (free-box box-t-c)
				   (free-box box-t+1-c)
				   score))
				 boxes-t+1))
			   boxes-t)))
	(free-c-flow-struct c-flow-struct-t)
	scores))
      (but-last boxes-movie)
      (rest boxes-movie)
      (but-last flow-movie)))

(define (get-phi1 boxes-movie flow-movie objects feature-medoids scale)
 (map (lambda (boxes flow)
       (map (lambda (box)
	     (let* ((c-flow-struct (optical-flow->c-flow-struct flow))
		    (box-c (voc4-detection->box box))
		    (flow-x ((c-function
			      double
			      ("average_flow_x_in_box" pointer pointer double))
			     box-c c-flow-struct scale))
		    (flow-y ((c-function
			      double
			      ("average_flow_y_in_box" pointer pointer double))
			     box-c c-flow-struct scale))
		    (magnitude (distance (vector 0 0) (vector flow-x flow-y)))
		    (orientation (atan flow-y flow-x))
		    (magnitude (quantize1 'velocity feature-medoids magnitude))
		    (orientation (quantize1 'orientation feature-medoids orientation))
		    (features (list
			       (position (voc4-detection-model box) objects)
			       magnitude
			       orientation)))
	      (free-box box-c)
	      (free-c-flow-struct c-flow-struct)
	      features))
	    boxes))
      boxes-movie
      flow-movie))

(define (get-phi2 boxes-movie feature-medoids scale)
 (map (lambda (boxes)
       (map (lambda (box-i)
	     (map (lambda (box-j)
		   (list
		    (quantize1 'x-distance
			       feature-medoids
			       (abs (- (x (voc4-detection-center box-i))
				       (x (voc4-detection-center box-j)))))
		    (- 1 (quantize1 'box-ratio
				    feature-medoids
				    (/ (voc4-detection-area box-i)
				       (voc4-detection-area box-j))))
		    (cond ((< (x (voc4-detection-center box-i))
			      (- (x (voc4-detection-center box-j)) 50))
			   0)
			  ((> (x (voc4-detection-center box-i))
			      (+ (x (voc4-detection-center box-j)) 50))
			   1)
			  (else 2))))
		   boxes))
...
(define (get-phi0 boxes-movie feature-medoids scale)
 (map (lambda (boxes)
       (map (lambda (box-i)
	     (map (lambda (box-j0)
		   (let ((xx (- (x (voc4-detection-center box-i))
				(x (voc4-detection-center box-j0))))
			 (yy (- (y (voc4-detection-center box-i))
				(y (voc4-detection-center box-j0)))))
		   (list
		    (quantize1 'displace-magnitude feature-medoids
			      (distance (vector 0 0) (vector xx yy)))
		    (quantize1 'displace-orientation feature-medoids
			      (atan yy xx)))))
		  (first boxes-movie)))
	    boxes))
      boxes-movie))

(define (quantize1 feature medoids x)
 (let* ((medoid (second (assq feature medoids)))
	(kind (third (assq feature medoids)))
	(distances
	 (map (lambda (mu-sigma-ignore)
	       (let ((mu (first mu-sigma-ignore))
		     (sigma (second mu-sigma-ignore)))
		(if (zero? kind)
		    ;; Find out whether there is another cluster beween x and
		    ;; mean[n]. This is yu239's hack to deal with the fact that
		    ;; the sigmas are inaccurate.
		    (if (some (lambda (other-mu)
			       (or (< mu other-mu x) (< x other-mu mu)))
			      (map first medoid))
			infinity
			(abs (/ (- x mu) (sqrt sigma))))
		    (angle-separation x mu))))
	      medoid)))
  (positionv (reduce min distances infinity) distances)))

(define (viterbi-sentence-track-in-c sentence-machines n-tracks roles boxes-movie
				     flow-movie scale top-n objects feature-medoids final-state?)
  ...
  (let* ((sentence (with-c-pointers
	 ...
	 (roles (map (lambda (r)
		      (let ((r-vec (allocate-ivec (length r))))
		       ...
		       r-vec))
		     roles))
	 (bss (get-bss boxes-movie))
	 (bps (get-bps boxes-movie flow-movie scale))
	 (phi1 (get-phi1 boxes-movie flow-movie objects feature-medoids scale))
	 (phi2 (get-phi2 boxes-movie feature-medoids scale))
	 (phi0 (get-phi0 boxes-movie feature-medoids scale))
	 (roles-c (easy-ffi:longunsigned-to-c 1 roles))
	 (bss-c (easy-ffi:double-to-c 2 bss))
	 (bps-c (easy-ffi:double-to-c 3 bps))
	 (phi1-c (easy-ffi:int-to-c 3 phi1))
	 (phi2-c (easy-ffi:int-to-c 4 phi2))
	 (phi0-c (easy-ffi:int-to-c 4 phi0))
   ...
   (easy-ffi:free 1 roles roles-c)
   (easy-ffi:free 2 bss bss-c)
   (easy-ffi:free 3 bps bps-c)
   (easy-ffi:free 3 phi1 phi1-c)
   (easy-ffi:free 4 phi2 phi2-c)
   (easy-ffi:free 4 phi0 phi0-c)
   (free box-sequence-c)
   result)))
...
(define (*new4-new3-corpus:semantics* lexical-entry)
 (*new3-corpus:semantics* lexical-entry))
	...
	(alike
	 (lambda (agent patient)
	  (and (equal? (voc4-detection-model (first agent))
        ...
        (picking-up
         (lambda (scale transformations agent patient . _)
          (and (equal? (voc4-detection-model (first agent)) "person")
	     (not (alike agent patient))
	     (stationary-f agent transformations scale)
	     (moving-direction-f patient transformations scale (- half-pi)))))
        (putting-down
         (lambda (scale transformations agent patient . _)
        ...
        (carrying
         (lambda (scale transformations agent patient . _)
	  (and (not (alike agent patient))
	     (equal? (voc4-detection-model (first agent)) "person")
	     (or (and (moving-direction-f agent transformations scale 0)
		   (moving-direction-f patient transformations scale 0))
		(and (moving-direction-f agent transformations scale pi)
		   (moving-direction-f patient transformations scale pi))))))
  ...
  (unless *new4-corpus:models*
   (set! *new4-corpus:models*
	 (let* ((eps 1e-30)  ;; Garbage output probability
		...
		(moving-velocity-magnitude `(discrete ,eps ,(- 0.1 eps) 0.3 0.3 0.3))
		;;
		(no-displacement '(discrete 1 0))
		(almost-no-displacement '(discrete 0.999 0.001))
		(dummy-displacement '(discrete 0.5 0.5))
		(displacement '(discrete 0 1))
		;;
		(dummy-x-distance `(discrete ,@(map-n (const (/ 3)) 3)))
		(close-x-distance `(discrete 1 0 0))
		(far-x-distance `(discrete 0 0.01 0.99))
