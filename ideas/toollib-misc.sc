...
;; assumes hyperrectangle
(define (matrix-dimensions m)
 (let loop ((d '#()) (m m))
  (if (vector? m)
      (loop (vector-append d `#(,(vector-length m)))
	    (vector-ref m 0))
      d)))

;; only applies to square matrix
(define (matrix-trace m)
 (let* ((ds (vector->list (matrix-dimensions m)))
	(nd (length ds)))
  (unless (= (length (remove-duplicates ds)) 1)
   (panic "Matrix not hypersquare!"))
  (reduce + (map-n (lambda (i) (apply matrix-ref-nd m (map-n (lambda _ i) nd))) (first ds)) 0)))
...
(define (ellipse-fit ps) (panic "matlab"))
