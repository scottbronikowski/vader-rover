...
(define (interp-object-phrase->scored-points object-phrase
 ...
 (if (= 1 (length object-phrase))
     ...
     (let* (;; (primary-object (first object-phrase))
	    ...
	    ;;  (vector-ref
	    ;;   (vector-ref (x lexicon) (position primary-object-class *objects*))
	    ;; 			       ;;;CHANGE *objects2* BACK TO *objects* IF INTERPRETER INCLUDES THE- WITH OBJECT NAMES
	    ...
	    ;; 				   all-objects-classes))
	    (all-objects-instantiated (let loop ((all-objects-classes all-objects-classes)
						  (objects objects))
					(if (null? all-objects-classes)
					    '()
					    (let ((o (a-member-of objects)))
					    (cons o
						  (loop (rest all-objects-classes)
							(remove o objects)))))))
	    (all-objects-type-score
	     (map (lambda (foo bar)
		   (vector-ref
		    (vector-ref (x lexicon)
				(position foo *objects*)) ;;;CHANGE *objects2* BACK TO *objects* IF INTERPRETER INCLUDES THE- WITH OBJECT NAMES
	    ...
	    ;;   all-objects))
	    (score
	     (+ 0;;(log primary-object-type-score)
		(reduce
		 +
		 (map
		  (lambda (p pf)
		   (let* ((obj1-index (position (second p) all-objects-variables))
			  ...
			  (obj2-instantiated (list-ref all-objects-instantiated
						       obj2-index))
			  (obj1-score
			   (list-ref all-objects-type-score obj1-index))
			  (obj2-score
			   (list-ref all-objects-type-score obj2-index))
			 )
      ...
      (list ;; primary-object-instantiated
       (first all-objects-instantiated)
	    score))))
...
(define (interp-object-phrase->point object-phrase
				     objects
				     lexicon)
 (dtrace "object-phrase:" object-phrase)
 (let* ((scored-objects
	 (dtrace "scored-objects:"
			(all-values
...
(define (trace-from-sentence-floorplan-and-lexicon sentence floorplan lexicon f)
 (trace-from-parse-functions-step-by-step
  (interpretation->parse-functions
   (rename-things-in-logic (first (semantic-interpreter sentence)))
   floorplan lexicon)
  f
