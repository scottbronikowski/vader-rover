...
(define (clause? thing) (= (first thing) 4))
  ...
  (lambda (p)
   (let ((clauses
	  (list
	   ...
	   (make-clause (vector-ref p 1)
			(make-compound-term (make-q) (list (make-constant 1)))
			'())
	   (make-clause (- 1.0 (vector-ref p 1))
