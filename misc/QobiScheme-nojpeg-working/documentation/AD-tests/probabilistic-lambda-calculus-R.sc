...
(define (constant-expression? expression) (= (first expression) 0))
...
(define (variable-access-expression? expression) (= (first expression) 1))
...
(define (lambda-expression? expression) (= (first expression) 2))
  ...
  (lambda (p)
   (let ((tagged-distribution
	  (evaluate
	   ...
	   (list (make-binding
		  (make-x0) (boolean-distribution (vector-ref p 0) (make-x0)))
		 (make-binding
		  (make-x1) (boolean-distribution (vector-ref p 1) (make-x1)))
		 (make-binding
		  (make-if-procedure)
		  (singleton-tagged-distribution
		   (lambda (x)
